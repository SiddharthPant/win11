...
2. **`2-windows-after-install.md`** — first boot on Windows: WSL install, winget apps, Maple Mono
   NF font, Git defaults, Windows Terminal, Docker Desktop, Windows Update policy, MacBook ↔ PC
   SSH/SMB.
3. **`3-wsl-arch.md`** — inside WSL: Arch Linux bootstrap from its barebones root-only image
   (sudo, user account, default user), systemd, dev packages, Git, and Docker via Docker
   Desktop's WSL integration.

## Conventions

- Each guide is an ordered checklist; do the sections top to bottom.
- Section headings are unnumbered and referenced by name ("see **Docker Desktop**"), so inserting
